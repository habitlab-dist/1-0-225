(function(){
  ...
  train_multi_armed_bandit_for_goal = require('libs_backend/multi_armed_bandit_thompson').train_multi_armed_bandit_for_goal;
  ...
  */
  ...
  out$.thompsonsampling = thompsonsampling = async function(enabled_goals){
    var enabled_interventions, goals, output, goal_name, goal_info, interventions, available_interventions, res$, i$, len$, intervention, predictor, selected_intervention;
    if (enabled_goals == null) {
      enabled_goals = (await get_enabled_goals());
    }
    enabled_interventions = (await get_enabled_interventions());
  ...
  out$.novelty = novelty = async function(enabled_goals){
    var enabled_interventions, goals, output, goal_name, goal_interventions, min_intervention, i$, ref$, len$, intervention;
    if (enabled_goals == null) {
      enabled_goals = (await get_enabled_goals());
    }
    enabled_interventions = (await get_enabled_interventions());
    ...
    for (goal_name in enabled_goals) {
      ...
      min_intervention = {};
      for (i$ = 0, len$ = (ref$ = goals[goal_name].interventions).length; i$ < len$; ++i$) {
        intervention = ref$[i$];
        if (!is_intervention_enabled(intervention)) {
          continue;
        }
        if (goal_interventions[intervention] == null) {
          min_intervention.intervention = intervention;
          min_intervention.sessions = goal_interventions[intervention];
          break;
        } else if (min_intervention.intervention == null || goal_interventions[intervention] < min_intervention.sessions) {
          min_intervention.intervention = intervention;
          min_intervention.sessions = goal_interventions[intervention];
        }
      }
      if (min_intervention.intervention != null) {
